...
export const axiosTryCatch = async <TDataType>(
  ...
  try {
    ...
    if (_error instanceof AxiosError) {
      ...
      if (!response) {
        errorMessage =
          "Oops! Server not responding. Server is not running or is temporarily down.";
      } else {
        const { data, error } = axiosResponseSchema.safeParse(response.data);
        if (error) {
          errorMessage =
            "Oops! Invalid Response came. Unexpected response came from server.";
        } else {
          if (response.status === HttpStatusCode.Unauthorized) {
            ...
            errorMessage =
              data.error.message ||
              "Oops! Something went wrong. Please check the logs or try again later.";
          }
        }
      }
